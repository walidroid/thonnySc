...
# Step 2.6.7: Add tkinter to embeddable Python (ThonnyTN method)
# The embeddable Python used by the backend subprocess does NOT include tkinter.
# We copy tkinter files from the system Python so the backend can import tkinter.
Write-Host "`n[2.6.7] Adding tkinter to embeddable Python (ThonnyTN method)..." -ForegroundColor Cyan

$sysPython = python -c "import sys; print(sys.exec_prefix)" 2>$null
if ($sysPython) {
    Write-Host "System Python found at: $sysPython"
    
    # 1. Copy _tkinter.pyd, tk86t.dll, tcl86t.dll to Python/ root
    $dllsDir = Join-Path $sysPython "DLLs"
    foreach ($dll in @("_tkinter.pyd", "tk86t.dll", "tcl86t.dll")) {
        $src = Join-Path $dllsDir $dll
        if (Test-Path $src) {
            Copy-Item $src -Destination "Python\" -Force
    ...
    # 2. Copy Lib/tkinter/ → Python/Lib/tkinter/ (the Python tkinter module)
    $tkSrc = Join-Path $sysPython "Lib\tkinter"
    $tkDest = "Python\Lib\tkinter"
    if (Test-Path $tkSrc) {
        if (-not (Test-Path "Python\Lib")) { New-Item -ItemType Directory -Path "Python\Lib" -Force | Out-Null }
        Copy-Item $tkSrc -Destination $tkDest -Recurse -Force
        Write-Host "  Copied Lib/tkinter/" -ForegroundColor Green
    } else {
        Write-Host "  WARNING: Lib/tkinter not found at $tkSrc" -ForegroundColor Yellow
    }
    
    # 3. Copy tcl/tcl8.6/ and tcl/tk8.6/ → Python/tcl/tcl8.6/ etc.
    $tclSrc = Join-Path $sysPython "tcl"
    $tclDest = "Python\tcl"
    if (Test-Path $tclSrc) {
        if (-not (Test-Path $tclDest)) { New-Item -ItemType Directory -Path $tclDest -Force | Out-Null }
        foreach ($subDir in @("tcl8.6", "tk8.6")) {
            $sub = Join-Path $tclSrc $subDir
            if (Test-Path $sub) {
                Copy-Item $sub -Destination $tclDest -Recurse -Force
                Write-Host "  Copied tcl/$subDir" -ForegroundColor Green
            }
        }
    } else {
        Write-Host "  WARNING: tcl/ folder not found at $tclSrc" -ForegroundColor Yellow
    }
    
    # 4. Ensure Python/Lib is in sys.path for the embeddable Python
    #    The embeddable .pth file controls this (and already includes "import site")
    #    We add Lib to the path so tkinter module can be found
    $pthFile = Get-ChildItem "Python\python*._pth" -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($pthFile) {
        $pthContent = Get-Content $pthFile.FullName -Raw
        if ($pthContent -notmatch "Lib") {
            Add-Content $pthFile.FullName "`nLib"
            Write-Host "  Added 'Lib' to $($pthFile.Name)" -ForegroundColor Green
        } else {
            Write-Host "  'Lib' already in $($pthFile.Name)" -ForegroundColor Yellow
        }
    }
    
    # 5. Verify tkinter can be imported by the embeddable Python
    $tkCheck = & ".\Python\python.exe" -c "import tkinter; print('OK')" 2>$null
    if ($tkCheck -eq "OK") {
        Write-Host "tkinter successfully available in embeddable Python!" -ForegroundColor Green
    } else {
        Write-Host "WARNING: tkinter import still failing in embeddable Python. ESP32 backend may not work." -ForegroundColor Yellow
    }
} else {
    Write-Host "WARNING: Could not find system Python. tkinter will NOT be available in backend." -ForegroundColor Red
    Write-Host "Install Python 3.x with tkinter from python.org and ensure it's in PATH." -ForegroundColor Yellow
}
