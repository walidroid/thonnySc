...
class MainCPythonBackend(MainBackend):
    ...
    def _install_pyqt5_safety_hook(self):
        """
        Intercepts PyQt5.QtWidgets import via sys.meta_path and replaces each
        widget class with a Python subclass whose dangerous methods validate
        argument counts before calling the underlying C++ sip method.

        Without this, wrong-argument calls like setText("x", y) cause a native
        Windows crash (STATUS_STACK_BUFFER_OVERRUN, exit 0xC0000409) before
        Python can produce any traceback. With this hook, a proper TypeError
        is raised in Python with the exact file and line number.
        """
        ...
        class _PyQt5WidgetsSafetyFinder:
            """sys.meta_path finder that patches PyQt5.QtWidgets right after it loads."""
